...
/**
 * View for detach/reattach + setVisible E2E tests. Exposes buttons to:
 *   - detach the rail from its parent layout and reattach it
 *   - flip rail.setVisible
 *   - flip a specific item's setVisible
 *   - mutate popover settings while the rail is detached, so a follow-up
 *     reattach can verify the new settings actually took effect
 */
...
public class DetachReattachView extends VerticalLayout {
    ...
    public DetachReattachView() {
        ...
        SideNavRailItem admin =
                new SideNavRailItem("Admin", "/admin", VaadinIcon.COG.create());
        admin.addItem(new SideNavRailItem("Users", "/admin/users"));
        rail.addItem(dashboard, code, admin);

        railHolder.add(rail);

        Button toggleRail = new Button("Toggle rail mode",
                e -> rail.setRailMode(!rail.isRailMode()));
        toggleRail.setId("toggle-rail");

        Button detach = new Button("Detach rail",
                e -> railHolder.remove(rail));
        detach.setId("detach-rail");

        Button reattach = new Button("Reattach rail",
                e -> {
        ...
        Button toggleRailVisible = new Button("Toggle rail visible",
                e -> rail.setVisible(!rail.isVisible()));
        toggleRailVisible.setId("toggle-rail-visible");

        Button toggleCodeVisible = new Button("Toggle Code visible",
                e -> code.setVisible(!code.isVisible()));
        toggleCodeVisible.setId("toggle-code-visible");

        Button changeHoverDelay = new Button("Hover delay 750",
                e -> rail.setPopoverHoverDelay(750));
        changeHoverDelay.setId("change-hover-delay");

        HorizontalLayout buttons = new HorizontalLayout(
                toggleRail, detach, reattach,
                toggleRailVisible, toggleCodeVisible,
                changeHoverDelay);
