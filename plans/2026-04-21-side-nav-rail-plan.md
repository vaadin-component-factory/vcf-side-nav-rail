...
> **Status (2026-04-23):** MVP + phase 2 + phase 9.1 complete. Phase 9.2 (accessibility) complete on branch `phase9.2/accessibility`, not yet merged to `main`. Reactor `./mvnw clean verify` is green — **81/81** addon unit tests + **47/47** Playwright E2E tests pass.
>
> **Phase 9.2 additions — accessibility (complete on `phase9.2/accessibility`):**
>
> - **ARIA attributes synced with popover state.** `SideNavRailItem.applyAriaAttributes(boolean)` sets `aria-haspopup="menu"` + seeds `aria-expanded="false"` on root items with children while rail mode is active; both are cleared on exit. A popover `OpenedChangeListener` wired in `ensurePopover()` keeps `aria-expanded` in sync with actual open/close state via `syncAriaExpanded(boolean)`. The `populatePopover()` walk also tags every copied popover item (and descendants) with `role="menuitem"`. Spec §4.4.5 + §5.1. Tests: 7 Karibu unit cases in `AriaAttributesTest`.
> - **`setOpenOnFocus` gated on rail mode.** In Task 2, `SideNavRailItem.ensurePopover()` now seeds `Popover.setOpenOnFocus` from the owning rail's `isRailMode()` rather than the previous hardcoded `false`. A new `applyFocusTrigger(boolean)` helper pushes live mode changes to existing popovers. Outside rail mode the popover stays hover-only; inside rail mode keyboard focus auto-opens it, which is the prerequisite for the Tab-user discovery flow in §4.4.4. Tests: 3 Karibu unit cases in `FocusTriggerTest`.
> - **`tabindex="-1"` on nested items in rail mode.** Nested items are visually hidden in rail mode (labels at `max-width: 0`) but still in the DOM. Without intervention they remained in the browser's Tab order, so keyboard users landed on invisible items. `SideNavRail.applyNestedTabindex` walks every descendant of every root item on `setRailMode` toggles, setting `tabindex="-1"` while rail mode is active and removing it on exit. Root items keep their natural tab order. Tests: 4 Karibu unit cases in `NestedTabindexTest`.
> - **Client-side keyboard adapter (`frontend/side-nav-rail-keyboard.js`).** Single `@JsModule` on `SideNavRail`, initialised via `Page.executeJs` on attach. The module registers a global `window.vaadinAddonsSideNavRail.initKeyboardNavigation(rail)` (dynamic `import()` inside executeJs does not resolve reliably in the production bundle; the global-registration-at-@JsModule-load pattern is rock-solid). One delegated document-level `keydown` listener per rail, handles: Arrow-Up/Down (normal: visible-item walk stopping at boundaries, skipping collapsed subtrees; rail-mode on root: root-only walk; inside popover: level-scoped sibling walk), Arrow-Right (normal: expand or descend; rail-root: open popover + focus first item; inside popover: expand or descend into nested), Arrow-Left (normal: collapse or focus parent; inside popover: collapse or focus popover-parent; at popover top-level: close popover + focus rail-root), Escape (close popover from either focus-on-root or focus-inside-popover; focus returns to rail-root). All focus moves go through a `focusItem(item)` helper that reaches into `item.shadowRoot` to focus the inner `<a id="link">` — the `<vaadin-side-nav-item>` custom element itself doesn't accept focus. Spec §4.4.
> - **E2E Playwright coverage.** New view `KeyboardNavigationView` at `/keyboard-navigation` with three root items of varying depth (Dashboard leaf, Code with 2 flat children, Admin with a nested Users → Active/Archived subtree plus a leaf Roles). New spec `keyboard-navigation.spec.ts` with 23 cases organised into describe-blocks for smoke / normal-mode Up-Down / normal-mode Right-Left / rail-mode root-nav + Esc / rail-mode Arrow-Right entry + in-popover Up-Down / rail-mode popover tree navigation. Test helpers `focusItem(page, path)` (scoped to `#rail` to avoid strict-mode violations on popover clones with duplicate paths) and `expectFocusedPath(page, expected)` (reads `document.activeElement.closest('vaadin-side-nav-item')?.getAttribute('path')`). **Bundle-staleness note:** every JS-file change needed an explicit `rm -rf e2e/src/main/bundles/prod.bundle e2e/target` before `./mvnw -pl e2e verify` — same gotcha as §9.1's tooltip work. **Timing note:** after Esc-closed popover + Arrow-Right reopen, the overlay takes ~1 frame to populate content; `moveFocusRightOnRailRoot` uses a fast path (synchronous focus when overlay is already open) + slow path (rAF-polling up to ~500 ms for the reopen case) to keep back-to-back key presses responsive.
>
> Spec §4.4 (the keyboard map), §4.4.5 (ARIA), §5.1 (marker attributes list), §4.2 (setOpenOnFocus line) and §9.2 (bullets crossed out with pointers to the implementation sections) all updated in place. `phase9.2/accessibility` is 12 commits ahead of `main`.
>
