...
>   - **Row-height parity attempt (reverted).** A first try pinned `::part(link) { min-height: var(--lumo-size-m); justify-content: center }` in rail mode. In the browser it produced off-center icons and a broken active-highlight, so the override was removed. Spec §5.2 now has a "Known rough edge" note deferring this to a browser-checked Phase-2 fix.
