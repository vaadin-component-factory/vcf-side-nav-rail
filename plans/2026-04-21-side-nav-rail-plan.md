...
> - **Post-MVP polish round:** three coupled items found while exercising the component in the browser (commit `8f70228`):
>   - **Popover must close on inline-expand.** Originally the popover was only gated by rail + popover mode. If the user hovered the item (popover opens) and then clicked the chevron to inline-expand it, the popover lingered while the inline children also appeared. Fix: `SideNavRailItem` now listens for the `expanded-changed` DOM event, re-runs `applyPopoverGating`, and calls `popover.close()` if the item is no longer eligible. Spec §4.1 gained a "Live state transitions" paragraph and §4.2 was updated.
>   - **Row-height parity.** In rail mode the items were shorter than in normal mode because hiding the label removed the line-height. Added `vaadin-side-nav-item::part(link) { min-height: var(--lumo-size-m); justify-content: center }` under the `theme~="rail"` scope. Spec §5.2 CSS snippet updated.
...
>   - New regression test `popover closes when the item is expanded inline in normal mode` in `popover-collapsed-item.spec.ts`. Test suite now 9/9 green.
