...
# Also kill anything on port 8081
PID_ON_PORT=$(ss -tlpn | grep 8081 | sed -n 's/.*pid=\([0-9]*\).*/\1/p')
...
# Start server
SERVER_PORT=8081 ./mvnw spring-boot:run > /tmp/claude-server.log 2>&1 &
echo $! > /tmp/claude-server.pid
echo "Server starting on port 8081 (PID: $(cat /tmp/claude-server.pid))"
